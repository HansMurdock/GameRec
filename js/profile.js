...
function getDiscordUsernameFromToken() {
  try {
    const token = localStorage.getItem("token");
    if (!token) return null;
    const payload = JSON.parse(atob(token.split(".")[1]));
...
const userName = storedUser.name || getNameFromToken() || "Gamer";
const userEmail = storedUser.email || getEmailFromToken() || "";
const userDiscordUsername =
  storedUser.discordUsername || getDiscordUsernameFromToken() || "";

const USER = {
  name: userName,
  username: userDiscordUsername
    ? `@${userDiscordUsername}`
    : userEmail
      ? `@${userEmail.split("@")[0]}`
...
// Sync localStorage supaya konsisten
...
const reviewedGames = [
  {
    ...
    stars: 5,
    date: "Mar 10, 2024",
    text: "A masterpiece of game design. Every encounter feels meaningful, and the open world is the most impressive I have ever seen in an action RPG. FromSoftware has outdone themselves in every possible way.",
    helpful: 142,
  },
  {
    slug: "baldurs-gate-3",
    ...
    stars: 5,
    date: "Feb 20, 2024",
    text: "Larian Studios delivered the RPG of the decade. The depth of choices, the writing, and the sheer amount of content is staggering. Co-op is an absolute blast.",
    helpful: 98,
  },
  {
    slug: "cyberpunk-2077",
    ...
    stars: 3,
    date: "Jan 5, 2024",
    text: "After years of patches it is a good game, but the launch was a disaster that is hard to forget. Night City is stunning but the story feels hollow in places. Still worth playing now.",
    helpful: 67,
  },
];
...
const activityFeed = [
  {
    slug: "elden-ring",
    name: "Elden Ring",
    detail: "Reached Leyndell — 68h played",
    time: "2h ago",
    badge: "Playing",
    badgeClass: "ab-playing",
  },
  {
    slug: "baldurs-gate-3",
    name: "Baldur's Gate 3",
    detail: "Unlocked achievement: Illithid",
    time: "1d ago",
    badge: "Playing",
    badgeClass: "ab-playing",
  },
  {
    slug: "red-dead-redemption-2",
    name: "Red Dead 2",
    detail: "Completed 100% — 90h total",
    time: "3d ago",
    badge: "Finished",
    badgeClass: "ab-finished",
  },
  {
    slug: "hades",
    name: "Hades",
    detail: "Heat 32 clear — 45h played",
    time: "5d ago",
    badge: "Finished",
    badgeClass: "ab-finished",
  },
...
const achievements = [
  ...
  },
  {
    ...
    points: "+100 XP",
    locked: false,
    bg: "rgba(255,100,50,.12)",
  },
  {
    icon: "📚",
    name: "Bibliophile",
    desc: "Add 100+ games to your library",
    date: "Jun 2023",
    points: "+150 XP",
    locked: false,
  ...
  },
  {
    icon: "⚔️",
    name: "Souls Master",
    desc: "Finish all FromSoftware titles",
    date: "Dec 2023",
    points: "+300 XP",
    locked: false,
    bg: "rgba(200,100,255,.12)",
  },
  {
    icon: "🌍",
    name: "World Explorer",
    desc: "Play games from 10 different genres",
    date: "—",
    points: "+250 XP",
    locked: false,
    bg: "rgba(0,200,200,.12)",
  },
  {
    icon: "💎",
    name: "Perfectionist",
    desc: "Get 100% in 5 games",
    date: "—",
    points: "+500 XP",
    locked: true,
  ...
  },
  {
    icon: "🎯",
    name: "Speed Demon",
    desc: "Complete a game in under 5 hours",
    date: "—",
    points: "+200 XP",
    locked: true,
    bg: "rgba(255,255,255,.04)",
...
async function renderLibrary() {
  ...
  grid.innerHTML = "<div style='color:#8a9bb0;padding:20px;'>Memuat...</div>";

  const games = await fetchUserGames("library");

  if (games.length === 0) {
    grid.innerHTML =
      "<div style='color:#8a9bb0;padding:20px;'>Belum ada game di library.</div>";
    return;
  }
  ...
  games.forEach((g, i) => {
    ...
    card.innerHTML = `
      <div class="shelf-card-img-wrap">
        ...
        <div class="shelf-score">${g.gameScore || "?"}</div>
        <div class="shelf-status ${statusTagClass[g.status] || "playing-tag"}">${g.status || "Playing"}</div>
      </div>
      <div class="shelf-card-body">
        <div class="shelf-card-title">${g.gameName}</div>
        <div class="shelf-card-meta">${g.gameSlug}</div>
    ...
    makeClickable(card, g.gameSlug);
    loadThumb(g.gameSlug, uid, "shelf-card-img");
  });
}
...
async function renderWishlist() {
  ...
  const games = await fetchUserGames("wishlist");

  if (games.length === 0) {
    grid.innerHTML =
      "<div style='color:#8a9bb0;padding:20px;'>Belum ada game di wishlist.</div>";
    return;
  }
...
function renderReviews() {
  ...
  reviewedGames.forEach((r, i) => {
    ...
    const stars = Array.from(
      { length: 5 },
      (_, s) => `<span class="star${s < r.stars ? "" : " empty"}">★</span>`,
    ).join("");
...
// ── FETCH LIBRARY / WISHLIST DARI BACKEND ────────────
async function fetchUserGames(listType) {
  const token = localStorage.getItem("token");
  if (!token) return [];
  try {
    const res = await fetch(
      `http://localhost:5000/api/user/games/${listType}`,
      {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  renderStats();
  // Update stat library & wishlist dari backend
  fetchUserGames("library").then((games) => {
    document.getElementById("stat-library").textContent = games.length;
    const sidebarLib = document.querySelector(
      ".sidebar-nav-item[data-tab='library'] .sidebar-nav-count",
    );
    if (sidebarLib) sidebarLib.textContent = games.length;
  });
  fetchUserGames("wishlist").then((games) => {
    document.getElementById("stat-wishlist").textContent = games.length;
    const sidebarWish = document.querySelector(
      ".sidebar-nav-item[data-tab='wishlist'] .sidebar-nav-count",
    );
    if (sidebarWish) sidebarWish.textContent = games.length;
  });
