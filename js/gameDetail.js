...
async function renderHero(game) {
  ...
  document.getElementById("hero-score-num").className =
    `meta-score-num ${scoreClass(game.score)}`;
...
function renderSidebar(game) {
  ...
  plWrap.innerHTML = game.platforms
    .map(
      (p) => `<div class="platform-pill">${platformIcons[p] || ""} ${p}</div>`,
    )
    .join("");

  // Genres
  const gnWrap = document.getElementById("sidebar-genres");
  gnWrap.innerHTML = game.genres
    .map(
      (g) =>
...
// Ambil Slug Game dari URL (Misal: ?id=elden-ring)
const urlParams = new URLSearchParams(window.location.search);
const gameSlug = urlParams.get("game") || urlParams.get("id");

// --- 1. CEK STATUS LOGIN ---
function checkAuth() {
  const userData = localStorage.getItem("user");
  const commentArea = document.getElementById("auth-comment-area");
  const loginMsg = document.getElementById("login-to-comment");
...
document
  ...
  ?.addEventListener("click", async () => {
    ...
    if (!content.trim()) return alert("Isi dulu komentarnya!");
    ...
    try {
      ...
      console.error("Gagal kirim komen:", err);
    }
  });

// --- 3. FUNGSI TAMPILKAN KOMENTAR ---
async function loadComments() {
  const list = document.getElementById("comments-list");
  try {
    const res = await fetch(`http://localhost:5000/api/comments/${gameSlug}`);
    ...
    if (result.success && result.data.length > 0) {
      list.innerHTML = result.data
        .map(
          (c) => `
        <div style="background: rgba(255,255,255,0.03); padding: 15px; border-radius: 8px; margin-bottom: 12px; border-left: 3px solid #ff3e3e;">
          <div style="display:flex; justify-content:space-between; margin-bottom:5px;">
            <strong style="color: #fff; font-size: 14px;">${c.User?.name || "Gamer"}</strong>
            <span style="color: #555; font-size: 11px;">${new Date(c.createdAt).toLocaleDateString()}</span>
          </div>
          <p style="color: #8a9bb0; font-size: 13px; line-height: 1.5;">${c.content}</p>
        </div>
      `,
        )
        .join("");
      ...
      list.innerHTML =
        '<p style="color: #555; font-size: 13px;">Belum ada ulasan untuk game ini. Jadilah yang pertama!</p>';
    }
  } catch (err) {
    list.innerHTML = '<p style="color: red;">Gagal memuat komentar.</p>';
  }
}
...
async function addToList(listType) {
  ...
  const game = GAME_DB[gameSlug] || DEFAULT_GAME;
  const btn = document.getElementById(`btn-${listType}`);

  btn.disabled = true;
  btn.textContent = "Menambahkan...";

  try {
    const gameImage = (await fetchGameImg(game.slug || gameSlug)) || "";

    const res = await fetch("http://localhost:5000/api/user/games", {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        ...
        listType,
        status: listType === "library" ? "Playing" : null,
        progress: 0,
      }),
    });
    ...
    if (data.success) {
      btn.textContent =
        listType === "library" ? "✓ In Library" : "✓ In Wishlist";
      btn.style.background = "#00bfa5";
    } else {
      alert(data.message);
      btn.disabled = false;
      btn.textContent =
        listType === "library" ? "+ Add to Library" : "+ Wishlist";
    }
  } catch (err) {
    console.error(err);
