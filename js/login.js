...
function switchTab(tab) {
  ...
  if (tab === "login") {
    ...
    document.getElementById("form-subtitle").textContent =
      "Sign in to your GameRec account to continue.";
    ...
    document.getElementById("form-subtitle").textContent =
      "Join thousands of gamers. Free forever.";
    document.getElementById("switch-text").innerHTML =
...
function clearMessages() {
  document
    .querySelectorAll(".form-msg")
    .forEach((m) => m.classList.remove("error", "success"));
  document
...
async function handleLogin(e) {
  ...
  try {
    ...
    // Kembalikan tampilan tombol seperti aslinya
    ...
    if (data.success) {
      // Simpan data di browser (standar Fullstack)
      ...
      setTimeout(() => {
        window.location.href = "http://127.0.0.1:5500/GameRec/html/index.html";
      }, 1200);
    } else {
      // Error dari backend kamu (Misal: Password Salah)
      msg.textContent = "✕ " + (data.message || "Login failed");
      msg.classList.add("error");
    }
  } catch (error) {
    // Jika backend mati / server tidak merespon
    btn.classList.remove("loading");
    btn.innerHTML = `Sign In <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>`;
    const msg = document.getElementById("login-msg");
    msg.textContent = "✕ Connection failed. Make sure your Node.js is running!";
    msg.classList.add("error");
  }
}
...
async function handleRegister(e) {
  ...
  // Validasi bawaan temenmu tetap dipertahankan
  if (!firstName) {
    showError("reg-firstname", "reg-firstname-err", "Required.");
    valid = false;
  }
  if (!lastName) {
    showError("reg-lastname", "reg-lastname-err", "Required.");
    valid = false;
  }
  if (!validateEmail(email)) {
    showError("reg-email", "reg-email-err", "Enter a valid email.");
    valid = false;
  }
  if (password.length < 8) {
    showError("reg-password", "reg-pass-err", "Min 8 characters.");
    valid = false;
  }
  if (password !== confirm) {
    showError("reg-confirm", "reg-confirm-err", "Passwords do not match.");
    valid = false;
  }
  ...
  try {
    ...
    const msg = document.getElementById("register-msg");
    if (data.success) {
      msg.textContent = `✓ Account created! Welcome, ${firstName}!`;
      msg.classList.add("success");
      // Balik ke tab login setelah berhasil daftar
...
// ── HANDLE TOKEN DARI URL (Setelah OAuth redirect) ──────────────
function handleOAuthCallback() {
  ...
  if (error) {
    const msg = document.getElementById("login-msg");
    msg.textContent = "✕ Login OAuth gagal: " + error;
    msg.classList.add("error");
    return;
  }
  ...
  if (token) {
    try {
      // 1. HAPUS SESI LAMA (Agar tidak tertumpuk data Google/Discord sebelumnya)
      ...
      // 3. Ambil data user terbaru dari payload token
      const user = {
        id: payload.id,
        email: payload.email,
        name: payload.name, // Ini akan otomatis ambil nama Discord jika login via Discord
        image: payload.image, // Pastikan backend kamu mengirim 'image' di payload
        role: payload.role,
      };
...
async function handleSocialLogin(provider, userData) {
  try {
    ...
    if (data.success) {
      ...
      const msg = document.getElementById("login-msg");
      msg.textContent = "✕ " + (data.message || "Social login gagal");
      msg.classList.add("error");
    }
  } catch (error) {
    console.error("Gagal nyambung ke backend:", error);
    const msg = document.getElementById("login-msg");
    msg.textContent = "✕ Connection failed. Pastikan Node.js backend berjalan!";
    msg.classList.add("error");
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  handleOAuthCallback(); // ← Cek token dari URL duluan
  initImages();
  initInputListeners();
});
